...
public class ColorpickerWidgetTest {
    ...
    private OHLinkedPage linkedPageState1 = new OHLinkedPage();
    {
        ...
        item.setType(OHItem.TYPE_COLOR);
    ...
    public void testOn() throws InterruptedException {
        ...
        Assert.assertEquals("Up command wasn't sent", new Pair<>(WIDGET_ITEM, Constants.COMMAND_ON), command);
    ...
    public void testOff() throws InterruptedException {
        ...
        Assert.assertTrue("Command queue should have one command", commandQueue.size() == 1);
        Pair<String, String> command = commandQueue.remove();
        Assert.assertEquals("Up command wasn't sent", new Pair<>(WIDGET_ITEM, Constants.COMMAND_OFF), command);
        Assert.assertTrue("Command queue should have no commands", commandQueue.size() == 0);
    }
    ...
    public void testFlow() throws InterruptedException {
        ...
        onView(withId(R.id.btn_increment)).perform(ViewActions.click());
        onView(withId(R.id.btn_decrement)).perform(ViewActions.click());
        Assert.assertTrue("Command queue should have 2 command", commandQueue.size() == 2);
        Pair<String, String> command = commandQueue.remove();
        Assert.assertEquals("Up command wasn't sent", new Pair<>(WIDGET_ITEM, Constants.COMMAND_ON), command);
        command = commandQueue.remove();
        Assert.assertEquals("Down command wasn't sent", new Pair<>(WIDGET_ITEM, Constants.COMMAND_OFF), command);
    }
